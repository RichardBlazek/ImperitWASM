...
namespace ImperitWASM.Shared.Value
{
	public class Brain
	{
		...
		readonly IEnumerable<SoldierType> SoldierTypes;
		readonly IReadOnlyList<Region> Regions;
		public Brain(Player player, IEnumerable<SoldierType> soldierTypes, IReadOnlyList<Region> regions)
		{
			Player = player;
		...
		{
			return provinces.Select(province => provinces.NeighborsOf(province).Where(n => n.IsAllyOf(Player)).Sum(n => Clamp(n.DefensePower - enemies[n.RegionId] + defense[n.RegionId], 0, n.DefensePower))).ToArray();
		}
		...
		SoldierType BestDefender(int i) => SoldierTypes.Where(type => type.IsRecruitable(Regions[i])).OrderBy(type => Player.Money / type.Price * type.DefensePower).First();
		Ship? BestShip(int i) => SoldierTypes.OfType<Ship>().Where(type => type.IsRecruitable(Regions[i]) && type.Price <= Player.Money).OrderByDescending(type => type.Capacity).FirstOrDefault();
		...
		static (Player, Provinces) Thinking(Brain ai, IReadOnlyList<Player> players, Provinces provinces, Settings settings)
		{
			...
			{
				var (new_players, new_provinces) = command.Perform(doer.Player, players, provinces, settings, doer.SoldierTypes, doer.Regions);
				return (new Brain(new_players.First(p => p == doer.Player), doer.SoldierTypes, doer.Regions), provinces.With(new_provinces))!;
			}
