...
{
	[Newtonsoft.Json.JsonConverter(typeof(RatioConvertorNewtonsoft))]
	[System.Text.Json.Serialization.JsonConverter(typeof(RatioConvertorSystemText))]
	public readonly struct Ratio : IEquatable<Ratio>, IComparable<Ratio>
	{
		...
		public int Interest(int amount) => amount + amount * this;
		public int Discount(int amount) => (int)((long)amount * int.MaxValue / (int.MaxValue + value));
	}
}
